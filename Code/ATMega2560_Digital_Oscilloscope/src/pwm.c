...
void PWM_init(void)
{
     ...
     * (8-bit TOP) to get ~200 Hz at 16 MHz with prescaler 64, which avoids visible flicker
...
int PWM_set_prescaler(uint8_t timer_id, uint16_t prescaler)
{
    ...
    case 2:
        ...
        if (prescaler == 1)
            ...
            TCCR2B |= (1 << CS22); /* common mapping */
        else if (prescaler == 256)
        { /* some MCUs: CS22|CS21 ??? */
            return -1;
        }
        else if (prescaler == 1024)
        { /* maybe unsupported for Timer2 */
            return -1;
        }
        else
            return -1;
